...
class ImageCaptioningModel(nn.Module):
    ...
    def forward(self, images, captions=None):
        ...
        visual_features = self.get_last_hidden_state(images)
    ...
    def get_last_hidden_state(self, images):
        with torch.no_grad():
            x = self.vision_encoder.conv1(images)  # shape = [*, width, grid, grid]
            x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
            x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]  
            x = torch.cat([self.vision_encoder.class_embedding.to(x.dtype) + 
                        torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], 
                        dim=1)  # shape = [*, grid ** 2 + 1, width]
            x = x + self.vision_encoder.positional_embedding.to(x.dtype)
            ...
            last_hidden_state = x  # This is equivalent to HuggingFace's last_hidden_state
...
def train_one_epoch(model, train_loader, optimizer, epoch, val_loader, scheduler=None, save_steps=104):
    ...
    for step, (images, captions) in enumerate(progress_bar):
        ...
        if (step + 1) % save_steps == 0 and epoch >= 4:
...
def main():
    ...
    optimizer = torch.optim.Adam([p for p in model.projection.parameters()], lr=lr1)
    ...
    optimizer = torch.optim.Adam(all_params, lr=lr2)
...
if __name__ == "__main__":
    ...
    # model = model.float()
    # image = preprocess(Image.open("p3_images/bike_mean.jpg")).unsqueeze(0).to(device)
    ...
    #     cls_tokens = cls_token.reshape(1, 1, -1).expand(x.shape[0], 1, -1)
    ...
    #     visual_features = model.visual.ln_post(x)
